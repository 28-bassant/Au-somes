...
class TowerBuildingLevel1Stage1 extends StatefulWidget {
  final VoidCallback? onNextStage;

  const TowerBuildingLevel1Stage1({Key? key, this.onNextStage}) : super(key: key);

  @override
  State<TowerBuildingLevel1Stage1> createState() =>
      TowerBuildingLevel1Stage1State();
}

class TowerBuildingLevel1Stage1State extends State<TowerBuildingLevel1Stage1> {
  ...
  bool _firstShadowReplaced = false;  // أول Shadow (أول صورة) - يتم استبداله بـ Actor الأول
  bool _secondShadowReplaced = false; // ثاني Shadow (تالت صورة) - يتم استبداله بـ Actor التاني
  bool _deceptionSoundPlayed = false;

  // لتتبع الأصوات التي تم تشغيلها
  bool _firstSoundPlayed = false;
  bool _secondSoundPlayed = false;

  // التحكم في تفعيل Actor الأول (يُفعل بعد ما Actor التاني يتحط في مكانه)
  bool _firstActorEnabled = false;
  ...
  Future<void> _playDeceptionSound() async {
    if (_deceptionSoundPlayed) return;

    if (_activity?.deceptionInstructions != null &&
        _activity!.deceptionInstructions!.isNotEmpty) {
      final url = _activity!.deceptionInstructions![0];
      if (url.isNotEmpty) {
        _deceptionSoundPlayed = true;
        await _deceptionPlayer.stop();
        await _deceptionPlayer.play(UrlSource(url));

        // بعد تشغيل الصوت، نفعّل Actor الأول
        if (mounted) {
          setState(() {
            _firstActorEnabled = true;
          });
        }
      }
    }
  ...
  void _handleSecondActorDragEnd(
    ...
    if (shadowBox != null) {
      ...
      if (shadowRect.contains(actorCenter)) {
        setState(() {
          _secondShadowReplaced = true; // استبدال Shadow الثاني
        });
        ...
        // نشغل صوت deception ونفعّل Actor الأول
        _playDeceptionSound();
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    // Actors
    final firstActor = elements[1]; // أول Actor (تاني صورة) - يتحرك بعد ما يتفعّل
    final secondActor = elements[3]; // ثاني Actor (رابع صورة) - يتحرك أولاً
    ...
    return Scaffold(
      body: Stack(
        children: [
          ...
          // Shadow الثاني - يتغير إلى Actor التاني لما يتم وضعه
          Positioned(
          ...
          // Actor التاني (رابع صورة) - يتحرك أولاً (دائماً نشط)
          ...
          // Actor الأول (تاني صورة) - يظهر دائماً لكن غير قابل للسحب في البداية
          if (!_firstShadowReplaced)
            Positioned(
              left: 50 + actorSize +60,
              top: screenHeight * 0.6,
              child: _firstActorEnabled
                  ? Draggable<String>(
                data: firstActor.id!,
                feedback: Material(
                  color: Colors.transparent,
